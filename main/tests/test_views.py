...
class TestViews(TestCase):
    ...
    def test_address_delete_only_deletes_current_user_address(self):
        user1 = models.User.objects.create_user(
            "user1", "pw432joij"
        )
        user2 = models.User.objects.create_user(
        ...
        self.client.force_login(user2)
        response = self.client.delete(reverse("address_delete", args=[2]))
        self.assertEqual(response.status_code, 302)
        self.assertFalse(
            models.Address.objects.filter(user=user2).exists()
        )
        self.assertTrue(
            models.Address.objects.filter(user=user1).exists()
        )
